...
#'               file.output=FALSE,plot.style="CMplot",cutOff=0.05)
SEIR <- function(Y, X, GM = NULL, CV = NULL, maxStep = 10, selector = c("L0","L1","MCP","stepwise","bess","MARS","SCAD","lars"),
...
############################################################################
   ...
   SEIR.0000 <- function(){
    ...
    if (X.check == TRUE){
	indexSNP = apply(X,2,sd)!=0 #varSNP= Rfast::colVars(X),indexSNP=which(varSNP!=0),X<-X[, indexSNP]
    X<-X[, indexSNP]
  ...
  lm.vec <- compiler::cmpfun(function(y, X, CV){
	  ...
	  err.cv <- sqrt(diag(chol2inv(chol(crossprod(CV)))*as.numeric(crossprod(yt)/(n-k))))
      zvalue.cv = U2/err.cv
...
##############
    ...
    #if (selector == "stepwise") {requireNamespace(StepReg,quietly=TRUE)}
...
#=================== Extract b-value  ========================#
 if (is.null(chunk.num) || chunk.num == 1) {
	...
	#if (extraction == "min" || is.null(extraction)){Bx1<-apply(Bx1,1,min)}
    if (extraction == "mean" || extraction == "bagging-like"){Bx1<-Rfast::rowmeans(Bx1)}
	#if (extraction == "mean" || extraction == "bagging-like"){Bx1<-rowmeans(Bx1)}
	#if (extraction == "mean" || extraction == "bagging-like"){Bx1<-apply(Bx1,1,mean)}
...
if (is.null(chunk.num) || chunk.num == 1) {
	...
	#if (extraction == "min" || is.null(extraction)){SEx1<-apply(SEx1,1,min)}
    if (extraction == "mean" || extraction == "bagging-like") {SEx1 <- Rfast::rowmeans(SEx1)}
	if (extraction == "mean" || extraction == "bagging-like"){SEx1<-rowmeans(SEx1)}
	#if (extraction == "mean" || extraction == "bagging-like"){SEx1<-apply(SEx1,1,mean)}
...
if (is.null(chunk.num) || chunk.num == 1) {
	...
	#if (extraction == "min" || is.null(extraction)){Zx1<-apply(Zx1,1,min)}
    if (extraction == "mean" || extraction == "bagging-like") {Zx1<-Rfast::rowmeans(Zx1)}
	if (extraction == "mean" || extraction == "bagging-like"){Zx1<-rowmeans(Zx1)}
	#if (extraction == "mean" || extraction == "bagging-like"){Zx1<-apply(Zx1,1,mean)}
...
if (is.null(chunk.num) || chunk.num == 1) {
	...
	#if (extraction == "min" || is.null(extraction)){Px1<-apply(Px1,1,min)}
    if (extraction == "mean") {Px1 <- Rfast::rowmeans(Px1)}
	#if (extraction == "mean") {Px1 <-apply(Px1,1,mean)}
...
print(length(which(p.mat[a, ] < cutOff/ncol(X))))
 if (length(which(p.mat[a, ] < cutOff/ncol(X))) <= 1){
   ...
   colnames(X1)<-colnames(p.mat[, which.min(p.mat[a, ]),drop=FALSE])} else {
 if(length(which(p.mat[a, ] < cutOff/ncol(X))) < sqrt(yn/log10(yn))){
...
##LD Removed
   ...
   if (selector == "SCAD"){
   if (ncol(X1)==1 || length(name)==1){name=name}else{
   res <- SIS::SIS(X1, y, family = "gaussian", penalty="SCAD",iter=FALSE, tune = "bic", nfolds = 5)
   index<-res$ix
   remove(res)
   if (length(index)==0){name<- colnames(X[, which.min(p.mat[a, ])])}else{
   name<-colnames(X1)[index];print(length(name))
   rm(index)
   X11<-as.matrix(X1[,c(name)],ncol=length(name))
   #mydata<-cbind(y, X11)
   #colnames(mydata)<-c("y",colnames(X11))
   #mydata<-as.data.frame(mydata)
   res1<-StepReg::stepwise(cbind(y,X11), "y", selection = "bidirection",select="SL",sle = 0.01, sls = 0.01)
   #res1<-StepReg::stepwise(y~.,data=mydata, selection = "bidirection",select="SL",sle = 0.05, sls = 0.05)
   index11<- res1$variate[-1]
   name<- index11}
   }
   }
   ...
   #mydata<-cbind(y, X1)
   #colnames(mydata)<-c("y",colnames(X1))
   res<-StepReg::stepwise(cbind(y,X1), "y", selection = "bidirection",select="SL",sle = 0.01, sls = 0.01)
   index <- res$variate[-1]
   if (length(index)==0){name<-colnames(X[, which.min(p.mat[a, ])])}else{
   name<- index
   #print(length(name))
...
###### end of redundant SNPs removing procedure #######
 ...
 if (identical(test.name, name) != TRUE) {test.name<-name} else {countor = countor +1; if (countor == 2) {print(c("Calculation ended at ", a), quote=FALSE); break}}
   ...
   print(length(name))
   print(name)
   X1<-as.matrix(X[, c(name)], nrow=nrow(X)) #X1<-as.matrix(X[,c(name)])
   colnames(X1)<-name
   ...
   if(is.null(QTNname)){name<-name} else {
   name<-do.call(rbind,QTNname);name<-name[duplicated(name)];name<-unique(name)}
   name<-as.vector(name)
   print(length(name));print(name)
